...
<div class="row">
	<div class="span12">
		...
		<?php if($this->request->session()->read('Install.salt') && $this->request->session()->read('Install.seed')) : ?>
		...
		<?php else : ?>
			<?php if(!$this->request->session()->read('Install.salt')): ?>
				<div class="alert alert-info">
					You have to change your salt key!
				</div>
			<?php endif; ?>
			<?php if(!$this->request->session()->read('Install.seed')): ?>
				<div class="alert alert-info">
